...
from medera.models import patient,doctors
...
def register(request):
    if(request.method == "POST"):
        ...
        record = patient.objects.create(fname=body['fname'], lname=body['lname'],
                                        mail=body['mail'], phone=body['phone'], doctor=body['doctor'], user=body['user'], pass1=p)
...
def dreg(request):
    if(request.method == "POST"):
        ...
        p = body['pass1']
        p = (hashlib.md5(p.encode())).hexdigest()
        record = doctors.objects.create(fname=body['fname'], lname=body['lname'], college=body['college'], cgpa=body['cgpa'], degree=body['degree'],
                                        mail=body['mail'], phone=body['phone'], user=body['user'], pass1=p)
        print("Success account created")
        return HttpResponse("Success account created", status=201)
    else:
